...
// Posts covering both shapes rehype-img-attrs has to handle: markdown
// ![]() images (hast elements) and the migrated thumb-link HTML
// (raw nodes, which rehype-raw only parses after user plugins run).
const POSTS = [
  "/2013/html5-css3-javascript-only-photobooth-with-image-download/",
  "/2019/didnt-launch-my-startup/",
];

for (const path of POSTS) {
  test(`content images carry intrinsic dimensions: ${path}`, async ({
    page,
  }) => {
