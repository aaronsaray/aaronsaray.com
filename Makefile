...
# dev server the tests start; clean first is what catches it. The two
# checkers read dist/ and have no target, so this is a recipe: a rule
...
verify: ## The gate: clean, check, build, URL contract, links, lint, test
	$(MAKE) clean
	$(MAKE) check
	$(MAKE) build
