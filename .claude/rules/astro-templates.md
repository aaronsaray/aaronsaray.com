...
* **Never `{" "}`.** ESLint fails on it. Prettier writes it when text
  flows around an inline tag inside a `{cond && (...)}` expression, so
  that markup lives in its own component (`OldPostNotice.astro` is the
  pattern) and the expression renders the component.
